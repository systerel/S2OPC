...
{
    {
        ...
        {
            session_mgr_it__continue_iter_session(&session_mgr__l_continue, &session_mgr__l_session);
            session_core__getall_orphaned(session_mgr__l_session, &session_mgr__l_dom,
                                          &session_mgr__l_channel_config_idx);
            if ((session_mgr__l_dom == true) && (session_mgr__l_channel_config_idx == session_mgr__channel_config_idx))
            {
                session_core__client_gen_activate_orphaned_session_internal_event(session_mgr__l_session,
                                                                                  session_mgr__channel_config_idx);
            }
            session_core__getall_to_create(session_mgr__l_session, &session_mgr__l_dom,
                                           &session_mgr__l_channel_config_idx);
            if ((session_mgr__l_dom == true) && (session_mgr__l_channel_config_idx == session_mgr__channel_config_idx))
            {
...
{
    {
        ...
        session_core__get_session_state_or_closed(session_mgr__l_session, &session_mgr__l_session_state);
        message_in_bs__read_msg_resp_header_service_status(session_mgr__resp_header, &session_mgr__l_resp_status);
        if (session_mgr__l_resp_status == constants__e_sc_ok)
        {
            ...
            case constants__e_msg_session_create_resp:
                ...
                {
                    session_core__get_session_channel(session_mgr__l_session, &session_mgr__l_session_channel);
                    if (session_mgr__l_session_channel == session_mgr__channel)
                    {
                        ...
                        session_core__client_create_session_resp_sm(session_mgr__channel, session_mgr__l_session,
                                                                    session_mgr__l_session_token, session_mgr__resp_msg,
                                                                    &session_mgr__l_bret);
                        ...
                        {
                            session_core__getall_to_activate(session_mgr__l_session, &session_mgr__l_bret,
                                                             &session_mgr__l_session_user);
                            if ((session_mgr__l_bret == true) &&
                                (session_mgr__l_session_user != constants__c_user_indet))
                            {
                                session_core__reset_session_to_activate(session_mgr__l_session);
                                session_core__client_gen_activate_user_session_internal_event(
                                    session_mgr__l_session, session_mgr__l_session_user);
                            }
                        }
                        else
                        {
            ...
            case constants__e_msg_session_activate_resp:
                    ...
                    (session_mgr__l_session_state == constants__e_session_scActivating))
                {
                    ...
                    if (session_mgr__l_session_channel == session_mgr__channel)
                    {
                        session_core__client_activate_session_resp_sm(session_mgr__channel, session_mgr__l_session,
                                                                      session_mgr__resp_msg, &session_mgr__l_bret);
                    }
                    else
                    {
                        ;
            ...
            case constants__e_msg_session_close_resp:
                if (session_mgr__l_session_state == constants__e_session_closing)
                {
                    ...
                    if (session_mgr__l_session_channel == session_mgr__channel)
                    {
                        ...
                        session_core__client_close_session_resp_sm(session_mgr__channel, session_mgr__l_session,
...
{
    {
        ...
        *session_mgr__service_ret = constants__c_StatusCode_indet;
        ...
        case constants__e_msg_session_create_req:
            session_core__server_create_session_req_and_resp_sm(session_mgr__channel, session_mgr__req_msg,
                                                                session_mgr__resp_msg, session_mgr__session,
                                                                session_mgr__service_ret);
            if (*session_mgr__service_ret == constants__e_sc_ok)
            {
                session_core__server_session_timeout_start_timer(*session_mgr__session, session_mgr__resp_msg);
            }
            break;
        case constants__e_msg_session_activate_req:
            session_core__server_get_session_from_token(session_mgr__session_token, session_mgr__session);
            session_core__is_valid_session(*session_mgr__session, &session_mgr__l_valid_session);
            session_core__get_session_state_or_closed(*session_mgr__session, &session_mgr__l_session_state);
            if (session_mgr__l_valid_session == true)
            {
                if ((((session_mgr__l_session_state == constants__e_session_created) ||
                    ...
                    (session_mgr__l_session_state == constants__e_session_userActivated))
                {
                    message_in_bs__read_activate_req_msg_user(session_mgr__req_msg, &session_mgr__l_is_valid_user_token,
                                                              &session_mgr__l_user);
                    if (session_mgr__l_is_valid_user_token == true)
                    {
                        session_core__is_valid_user(session_mgr__l_user, &session_mgr__l_valid_user);
                        if (session_mgr__l_valid_user == true)
                        {
                            session_core__server_activate_session_req_and_resp_sm(
                                session_mgr__channel, *session_mgr__session, session_mgr__l_user, session_mgr__req_msg,
                                session_mgr__resp_msg, session_mgr__service_ret);
                        }
                        else
                        {
        ...
        case constants__e_msg_session_close_req:
            ...
            if (session_mgr__l_valid_session == true)
            {
                if (((session_mgr__l_session_state == constants__e_session_created) ||
                    ...
                    (session_mgr__l_session_state == constants__e_session_userActivated))
                {
                    session_core__get_session_channel(*session_mgr__session, &session_mgr__l_session_channel);
                    if (session_mgr__l_session_channel == session_mgr__channel)
                    {
                        session_core__server_close_session_req_and_resp_sm(session_mgr__channel, *session_mgr__session,
                                                                           session_mgr__req_msg, session_mgr__resp_msg,
                                                                           session_mgr__service_ret);
                    }
                    else
                    {
            ...
            break;
        default:
            *session_mgr__service_ret = constants__e_sc_bad_service_unsupported;
            break;
        }
    }
}
...
{
    {
        ...
        *session_mgr__channel = constants__c_channel_indet;
        ...
        {
            session_core__client_get_token_from_session(session_mgr__session, session_mgr__session_token);
            session_core__get_session_channel(session_mgr__session, session_mgr__channel);
            session_request_handle_bs__client_add_session_request_handle(session_mgr__session, session_mgr__req_handle);
...
{
    {
        ...
        *session_mgr__session = constants__c_session_indet;
        session_request_handle_bs__client_get_session_and_remove_request_handle(session_mgr__req_handle,
                                                                                &session_mgr__l_session);
        session_core__is_valid_session(session_mgr__l_session, &session_mgr__l_valid_session);
        session_core__get_session_state_or_closed(session_mgr__l_session, &session_mgr__l_session_state);
        if (session_mgr__l_valid_session == true)
        {
            session_core__get_session_channel(session_mgr__l_session, &session_mgr__l_session_channel);
            ...
            {
                session_core__is_session_valid_for_service(session_mgr__channel, session_mgr__l_session,
                                                           session_mgr__bres);
            }
            else
            {
...
{
    {
        ...
        request_handle_bs__req_handle_do_nothing(session_mgr__req_handle);
        session_core__server_get_session_from_token(session_mgr__session_token, &session_mgr__l_session);
        session_core__is_valid_session(session_mgr__l_session, &session_mgr__l_valid_session);
        session_core__get_session_state_or_closed(session_mgr__l_session, &session_mgr__l_session_state);
        *session_mgr__session = constants__c_session_indet;
        if (session_mgr__l_valid_session == true)
        {
                ...
                (session_mgr__l_session_channel == session_mgr__channel))
            {
                ...
                *session_mgr__status_code_err = constants__e_sc_ok;
                *session_mgr__session = session_mgr__l_session;
            ...
            else
            {
                ...
                *session_mgr__is_valid_res = false;
                if (session_mgr__l_session_channel != session_mgr__channel)
                {
                    *session_mgr__status_code_err = constants__e_sc_bad_secure_channel_id_invalid;
                }
                else
                {
                    if (session_mgr__l_session_state == constants__e_session_created)
                    {
                        *session_mgr__status_code_err = constants__e_sc_bad_session_not_activated;
                    }
                    else
                    {
                        *session_mgr__status_code_err = constants__e_sc_bad_invalid_state;
                    }
                }
            }
        }
        ...
        {
            *session_mgr__is_valid_res = false;
            *session_mgr__status_code_err = constants__e_sc_bad_session_id_invalid;
        }
    }
}

void session_mgr__server_validate_session_service_resp(const constants__t_channel_i session_mgr__channel,
...
{
    {
        ...
        if (session_mgr__l_valid_session == true)
        {
            ...
            if ((session_mgr__l_session_state == constants__e_session_userActivated) &&
                (session_mgr__l_session_channel == session_mgr__channel))
            {
                session_core__is_session_valid_for_service(session_mgr__channel, session_mgr__session,
                                                           session_mgr__is_valid_res);
                *session_mgr__status_code_err = constants__e_sc_ok;
            }
            else
...
{
    {
        ...
        if (session_mgr__l_valid_session == true)
        {
            ...
            {
                session_core__client_create_session_req_sm(session_mgr__session, session_mgr__channel,
                                                           session_mgr__create_req_msg, &session_mgr__l_valid);
...
{
    {
        ...
        if (session_mgr__l_session_state == constants__e_session_init)
        {
            *session_mgr__bres = true;
            session_core__set_session_to_create(session_mgr__l_session, session_mgr__channel_config_idx);
            session_core__set_session_to_activate(session_mgr__l_session, session_mgr__user, session_mgr__app_context);
        }
        else
        {
            *session_mgr__bres = false;
...
{
    {
        ...
        if (session_mgr__l_valid_session == true)
        {
            ...
            if ((session_mgr__l_session_state == constants__e_session_created) ||
                (session_mgr__l_session_state == constants__e_session_userActivated))
            {
                session_core__client_user_activate_session_req_sm(session_mgr__session, session_mgr__user,
                                                                  session_mgr__activate_req_msg, &session_mgr__l_ret,
                                                                  session_mgr__channel, session_mgr__session_token);
...
{
    {
        ...
        if (session_mgr__l_valid_session == true)
        {
            ...
            if ((session_mgr__l_session_state == constants__e_session_scOrphaned) ||
                (session_mgr__l_session_state == constants__e_session_userActivated))
            {
                session_core__client_sc_activate_session_req_sm(session_mgr__session, session_mgr__channel,
...
{
    {
        ...
        if (session_mgr__l_valid_session == true)
        {
            ...
            if (((session_mgr__l_session_state == constants__e_session_created) ||
                ...
                (session_mgr__l_session_state == constants__e_session_userActivated))
            {
                session_core__client_close_session_req_sm(session_mgr__session, session_mgr__close_req_msg,
                                                          &session_mgr__l_ret, session_mgr__channel,
                                                          session_mgr__session_token);
                if (session_mgr__l_ret != constants__e_sc_ok)
                {
...
{
    {
        ...
        session_core__server_session_timeout_evaluation(session_mgr__session, &session_mgr__l_session_expired);
