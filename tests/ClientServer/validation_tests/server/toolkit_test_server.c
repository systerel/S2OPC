                                              ...
                                              SOPC_UserAuthentication_Status* authenticated)
{
    ...
    *authenticated = SOPC_USER_AUTHENTICATION_REJECTED_TOKEN;
    ...
    if (&OpcUa_X509IdentityToken_EncodeableType == token->Body.Object.ObjType)
    {
        ...
        /* TODO:
            After the peer review, start the refactor and add SOPC_PKIProviderNew_CheckLeafCertificate in
...
#else
    status = SOPC_PKIProviderNew_CreateFromStore("./test_user_PKI", &pX509_UserIdentity_PKI);
