...
START_TEST(test_crypto_symm_sign_B256)
{
    ...
    memset(output, 0, sizeof(output));
    memset(hexoutput, 0, sizeof(hexoutput));
    ck_assert(SOPC_CryptoProvider_SymmetricSign(crypto, input, 64, pSecKey, output, 20) == STATUS_OK);
    ck_assert(hexlify(output, hexoutput, 20) == 20);
    ck_assert(memcmp(hexoutput, "2b0aee11aa84cb5ddae7b5ef9c46c4a249e5b981", 40) == 0);

    // Check verify
    ck_assert(SOPC_CryptoProvider_SymmetricVerify(crypto, input, 64, pSecKey, output, 20) == STATUS_OK);
    output[1] ^= 0x20; // Change 1 bit
    ck_assert(SOPC_CryptoProvider_SymmetricVerify(crypto, input, 64, pSecKey, output, 20) == STATUS_NOK);
...
START_TEST(test_crypto_derive_lengths_B256)
{
    ...
    // Check sizes
    ck_assert(SOPC_CryptoProvider_DeriveGetLengths(crypto, &lenCryptoKey, &lenSignKey, &lenIV) == STATUS_OK);
...
START_TEST(test_crypto_derive_data_B256)
{
    ...
    ck_assert(SOPC_CryptoProvider_SymmetricGetLength_CryptoKey(crypto, &lenSecr) == STATUS_OK); // TODO: use future GetLength_Nonce
    lenSeed = lenSecr;
    ...
    ck_assert(SOPC_CryptoProvider_DerivePseudoRandomData(crypto, secret, lenSecr, seed, lenSeed, output, 64) == STATUS_OK);
    ...
    ck_assert(SOPC_CryptoProvider_DerivePseudoRandomData(crypto, secret, lenSecr, seed, lenSeed, output, lenOutp) == STATUS_OK);
    ...
    ck_assert(SOPC_CryptoProvider_DerivePseudoRandomData(crypto, secret, lenSecr, seed, lenSeed, output, 1024) == STATUS_OK);
...
START_TEST(test_crypto_derive_keysets_B256)
{
    ...
    ck_assert(lenOutp < 1024);
    ck_assert(SOPC_CryptoProvider_SymmetricGetLength_CryptoKey(crypto, &lenCliNonce) == STATUS_OK); // TODO: use future GetLength_Nonce
    ...
    ck_assert(SOPC_CryptoProvider_DeriveKeySets(crypto, clientNonce, lenCliNonce, serverNonce, lenSerNonce, &cliKS, &serKS) == STATUS_OK);
...
static inline void setup_certificate(void)
{
    ...
    ck_assert(unhexlify("308204bb308202a3a003020102020102300d06092a864886f70d01010b0500308188310b3009060355040613024652310c300a06035504080c03494446310e30"
                        ...
                        "5043532043413122302006092a864886f70d0109011613696e676f70637340737973746572656c2e6672301e170d3137303532323132343431335a170d313830"
                        "3532323132343431335a3057310b3009060355040613024652310c300a06035504080c03494446310e300c06035504070c0550415249533111300f060355040a"
                        ...
                        "2914cf210203010001a360305e301d0603551d0e04160414a3f8e031d1f6f412bace4ddf0eeb62da209d3c79301f0603551d2304183016801478a2ae09c22875"
                        "23d9b20f7fb31293ecf5ce2d14300f0603551d130101ff040530030101ff300b0603551d0f040403020106300d06092a864886f70d01010b0500038202010033"
                        "75d0fef58f5d7fcea8ac5a7aa9f94aacab123925f0298ee5b8f81df188e149df1e7771539437bef947dcd90dc12bd42ae185e715d7633ba4386d99e39e11c012"
                        "b998a2f127a6e515f7fa657346518332f00ae4320d03d461f6bff99240e54093c6bf98f24747f9a2080ea6391d6fd34f85a7daae1c1c10c21129950542f89715"
                        "ef2fc11d2c73982bdbbeac58627b13c8702ee9dacccd4b8b903bc9834beb4c898d8a70b323dadd7db8235146c0ed0aa26eda611a0fb1c5ea7cd97928317d6735"
                        "945dbe16fb72c5477b9403a7e34e7528090e37543e988fdf3fc72669991c8161b6e8cd9231c987a8bd35541d646a6ff3146b229f53b8c650df69d1aa7b292076"
                        "25c443a8f4fc87cb848abc5937c1b230d9b940386f11c35318659ef6712424f7d21943fe7ad3b6acbe15cd903a33b396c755661967a31026576220a547b43e3f"
                        "8f2e4df13c14f8250c02a56b4add9fc1f503bc7d8a8573909d8e9d3c5a008be8170b528711cc0084072ad6839b52881e613c9c8121415a4c8bcdd6e4d448616e"
                        "003addd166727c198735601abd2526b2fe9cd3158df9b258ef3101f8ddeaf2b36e8d7f6348dea70e3f5d69f8e1b4ebe8156405f55692e9342630264f1dc948a4"
                        "095da188cd50e7f8cd30fce5232cb564dc0bef2d761e12f5dde9ac93a81f67fae30d251a59f578dd6aaa7d3edbc81387dae9d67ee148d1662ca139446a8de3", der_cert, 1215) == 1215);
...
START_TEST(test_crypto_asym_lengths_B256)
{
    ...
    ck_assert(128 == len);
    ck_assert(SOPC_CryptoProvider_AsymmetricGetLength_Decryption(crypto, key_priv, 128, &len) == STATUS_OK);
    ck_assert(86 == len);
    ck_assert(SOPC_CryptoProvider_AsymmetricGetLength_Encryption(crypto, key_pub, 688, &len) == STATUS_OK);
    ck_assert(1024 == len);
...
START_TEST(test_crypto_asym_crypt_B256)
{
    ...
    ck_assert(SOPC_CryptoProvider_AsymmetricEncrypt(crypto, input, 32, key_pub, output, 128) == STATUS_OK);
    ck_assert(SOPC_CryptoProvider_AsymmetricDecrypt(crypto, output, 128, key_priv, input_bis, 86, &len) == STATUS_OK);
    ck_assert(len == 32);
    ...
    ck_assert(SOPC_CryptoProvider_DerivePseudoRandomData(crypto, clientNonce, 32, serverNonce, 32, input+32, 688-32) == STATUS_OK);
    ck_assert(SOPC_CryptoProvider_AsymmetricEncrypt(crypto, input, 688, key_pub, output, 1024) == STATUS_OK);
    ck_assert(SOPC_CryptoProvider_AsymmetricDecrypt(crypto, output, 1024, key_priv, input_bis, 688, &len) == STATUS_OK);
    ck_assert(len == 688);
...
START_TEST(test_crypto_asym_sign_verify_B256)
{
    ...
    memset(sig, 0, 128);
    strncpy((char *)input, "Test INGOPCS Test", 32); // And test padding btw...
    ck_assert(SOPC_CryptoProvider_AsymmetricSign(crypto, input, 32, key_priv, sig, 128) == STATUS_OK);
    ck_assert(SOPC_CryptoProvider_AsymmetricVerify(crypto, input, 32, key_pub, sig, 128) == STATUS_OK);
    // b) Multiple messages (> 214, and as output is 1024, < 856)
    //  Using previously generated nonce, to fill input[32:856]
    ck_assert(unhexlify("c3cc8578608ae88e9690b921254d028e1b9cdc75fbf5070c4e39e5712b4a8bdf", clientNonce, 32) == 32);
    ck_assert(unhexlify("9b8a2d541f4b3ed8ae69111cc85c4ea875fb7e2a541aa87d703fe1a5d037dcfc", serverNonce, 32) == 32);
    ck_assert(SOPC_CryptoProvider_DerivePseudoRandomData(crypto, clientNonce, 32, serverNonce, 32, input+32, 688-32) == STATUS_OK);
    ck_assert(SOPC_CryptoProvider_AsymmetricSign(crypto, input, 688, key_priv, sig, 128) == STATUS_OK);
    ck_assert(SOPC_CryptoProvider_AsymmetricVerify(crypto, input, 688, key_pub, sig, 128) == STATUS_OK);
}
END_TEST
...
START_TEST(test_crypto_asym_uri_B256)
{
    ck_assert(strncmp(SOPC_CryptoProvider_AsymmetricGetUri_SignAlgorithm(crypto),
                      ...
                      strlen(SOPC_SecurityPolicy_Basic256_URI_SignAlgo)+1)
              == 0);
}
END_TEST
...
static inline void setup_pki_stack(void)
{
    ...
    ck_assert(unhexlify("308205f5308203dda003020102020900a5d3e9dfcf8e3f74300d06092a864886f70d01010b0500308188310b3009060355040613024652310c300a0603550408"
