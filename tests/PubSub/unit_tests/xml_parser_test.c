...
START_TEST(test_pub_xml_parsing)
{
    ...
    ck_assert_int_eq(-1, SOPC_FieldMetaData_Get_ValueRank(fieldMetaData));
    ck_assert_ptr_null(SOPC_FieldMetaData_Get_ArrayDimension(fieldMetaData)->arrayDimensions);
    ...
    ck_assert_int_eq(1, SOPC_FieldMetaData_Get_ValueRank(fieldMetaData));
    const SOPC_PubSub_ArrayDimension* arrayDimensions = SOPC_FieldMetaData_Get_ArrayDimension(fieldMetaData);
    ck_assert_ptr_nonnull(arrayDimensions->arrayDimensions);
    ck_assert_int_eq(0, arrayDimensions->arrayDimensions[0]);
    ...
    arrayDimensions = SOPC_FieldMetaData_Get_ArrayDimension(fieldMetaData);
    ck_assert_ptr_nonnull(arrayDimensions->arrayDimensions);
    ck_assert_int_eq(2, arrayDimensions->arrayDimensions[0]);
    ck_assert_int_eq(6, arrayDimensions->arrayDimensions[1]);
    ck_assert_int_eq(0, arrayDimensions->arrayDimensions[2]);
...
START_TEST(test_pub_acyclic_xml_parsing)
{
    ...
    const uint16_t nb_writerGroups = SOPC_PubSubConnection_Nb_WriterGroup(conn);
    ck_assert_uint_eq(2, nb_writerGroups);
    const bool isAcyclic = SOPC_PubSubConnection_Get_AcyclicPublisher(conn);
    ck_assert(isAcyclic);
    ...
    ck_assert_uint_eq(SOPC_SecurityMode_None, SOPC_WriterGroup_Get_SecurityMode(writerGroup));
    ck_assert_uint_eq(2, SOPC_WriterGroup_Nb_DataSetWriter(writerGroup));
    ck_assert_double_eq(1000., SOPC_WriterGroup_Get_KeepAlive(writerGroup));
    // DataSetWriter 1
    SOPC_DataSetWriter* dataSetWriter = SOPC_WriterGroup_Get_DataSetWriter_At(writerGroup, 0);
    ck_assert_ptr_nonnull(dataSetWriter);
    ck_assert_uint_eq(50, SOPC_DataSetWriter_Get_Id(dataSetWriter));
...
int main(void)
{
    ...
    tcase_add_test(tc_pub_xml_parsing, test_pub_acyclic_xml_parsing);
