...
int main(void)
{
    ...
    if (SOPC_STATUS_OK == status)
    {
        ...
        status = (SOPC_UADP_NetworkMessage_Error_Code_None == errorCode) ? SOPC_STATUS_OK : SOPC_STATUS_NOK;
        ...
        if (SOPC_STATUS_OK == status && buffer != NULL && buffer_payload != NULL)
        {
            ...
            {
                fprintf(stderr, "Sign and encrypt and merge encoded buffers error: %08X\n", errorCode);
            }
        }
        else
        {
