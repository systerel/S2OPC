                                                                         ...
                                                                         void* param)
{
    ...
    SOPC_Variant* sgId = &inputArgs[0];
    if (SOPC_VariantArrayType_SingleValue != sgId->ArrayType)
    {
        /* Should not happen if method is well defined in address space */
        return OpcUa_BadInternalError;
    }

    if (0 != strcmp(SKS_SECURITY_GROUPID, SOPC_String_GetRawCString(&sgId->Value.String)))
    {
