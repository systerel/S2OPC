                                                       ...
                                                       SOPC_StatusCode* errorStatus)
{
    ...
    SOPC_Buffer* ackMsgBuffer = SOPC_Buffer_Create(SOPC_TCP_UA_ACK_MSG_LENGTH);
            ...
            epConfig->nbClientsToConnect > auxParam)
        {
            ...
            if (result)
            {
                ...
                SOPC_Sockets_EnqueueEvent(
                    SOCKET_CREATE_CONNECTION, connectionIdx,
                    (uintptr_t) SOPC_strdup(epConfig->clientsToConnect[auxParam].clientEndpointURL), 0);
            }
            else
            {
                SOPC_Logger_TraceError(
        ...
        if (result)
        {
            ...
            if (SOPC_STATUS_OK == status)
            {
                if (SC_CONNECT == event)
                {
                    ...
                    SOPC_Sockets_EnqueueEvent(SOCKET_CREATE_CONNECTION, idx, (uintptr_t) SOPC_strdup(scConfig->url), 0);
