...
#define /*lint -e(960,961)*/ UASPROV_SET_BIT(byte, bit) /*lint -e(835)*/ \
    ((byte) = (UAS_UInt8)((byte) BITOR(UAS_UInt8)(0x01u << (bit))))

/* This Makro is used to read the value of a single bit from a byte. */
#define /*lint -e(961)*/ UASPROV_GET_BIT(byte, bit) /*lint -e(835)*/ \
 ...
 */
UAS_UInt8 byUASPROV_Init(UASPROV_StateMachine_type* const pzStateMachine,
 ...
 * \return UASPROV_OK if ok, error code if error occured
 */
UAS_UInt8 byUASPROV_Reset(UASPROV_StateMachine_type* const pzStateMachine,
                          UASPROV_StateMachine_type* const r_pzStateMachine)
{
 ...
 * \return UASPROV_OK if ok, error code if error occured
 */
UAS_UInt8 byUASPROV_ChangeSPI(UASPROV_StateMachine_type* const pzStateMachine,
                              UASPROV_StateMachine_type* const r_pzStateMachine,
 ...
 * \return UASPROV_OK if ok, error code if error occured
 */
UAS_UInt8 byUASPROV_Start(UASPROV_StateMachine_type* const pzStateMachine,
                          UASPROV_StateMachine_type* const r_pzStateMachine)
{
    /* check the redundant instance data */
    UAS_UInt8 byRetVal = byUASPROV_CheckRedundantData(pzStateMachine, r_pzStateMachine);
    ...
    if (UASPROV_OK EQ byRetVal)
    {
        if ((UASPROV_S0_INITIALIZE EQ pzStateMachine->nState) && (NULL NOT_EQ pzStateMachine->pzInstanceData) &&
 ...
 * \return UASPROV_OK if ok, error code if error occured
 */
UAS_UInt8 byUASPROV_Stop(UASPROV_StateMachine_type* const pzStateMachine,
                         UASPROV_StateMachine_type* const r_pzStateMachine)
{
    /* check the redundant instance data */
    UAS_UInt8 byRetVal = byUASPROV_CheckRedundantData(pzStateMachine, r_pzStateMachine);
 ...
 * \return UASPROV_OK if ok, error code if error occured
 */
UAS_UInt8 byUASPROV_Execute(UASPROV_StateMachine_type* const pzStateMachine,
                            UASPROV_StateMachine_type* const r_pzStateMachine)
{
    /* check the redundant instance data */
    UAS_UInt8 byRetVal = byUASPROV_CheckRedundantData(pzStateMachine, r_pzStateMachine);
 ...
 * \return UASPROV_OK if ok, error code if error occured
 */
static UAS_UInt8 byUASPROV_StateWaitForRequest(UASPROV_StateMachine_type* const pzStateMachine,
                                               UASPROV_StateMachine_type* const r_pzStateMachine)
{
    UAS_UInt8 byRetVal = UASPROV_DEFAULT_ERR;
    ...
    case UASPROV_S1_WAIT_FOR_REQUEST:
    {
        ...
        if ((pzStateMachine->zRequestSpdu_i.dwSafetyConsumerId NOT_EQ pzRequestSPDU->dwSafetyConsumerId) OR(
                pzStateMachine->zRequestSpdu_i.dwMonitoringNumber NOT_EQ pzRequestSPDU->dwMonitoringNumber)
                OR(pzStateMachine->zRequestSpdu_i.byFlags NOT_EQ pzRequestSPDU->byFlags))
        {
#ifdef UASDEF_DBG
            ...
            pzOutputSAPI->bOperatorAckRequested =
                UASPROV_GET_BIT(pzRequestSPDU->byFlags, UAS_BITPOS_OPERATOR_ACK_REQUESTED);
 ...
 * \return UASPROV_OK if ok, error code if error occured
 */
static UAS_UInt8 byUASPROV_StatePrepareSpdu(UASPROV_StateMachine_type* const pzStateMachine,
                                            UASPROV_StateMachine_type* const r_pzStateMachine)
{
    UAS_UInt8 byRetVal = UASPROV_DEFAULT_ERR;
 ...
 * \param[in/out]  pzStateMachine   - Pointer to the primary state machine data
 */
static void vUASPROV_BuildResponseSPDU(const UASPROV_StateMachine_type* const pzStateMachine)
{
    ...
    if ((0uL EQ pzInputSAPI->zSafetyBaseId.dwData1) AND(0u EQ pzInputSAPI->zSafetyBaseId.wData2)
            AND(0u EQ pzInputSAPI->zSafetyBaseId.wData3) AND(0u EQ pzInputSAPI->zSafetyBaseId.abyData4[0])
                AND(0u EQ pzInputSAPI->zSafetyBaseId.abyData4[1]) AND(0u EQ pzInputSAPI->zSafetyBaseId.abyData4[2])
                    AND(0u EQ pzInputSAPI->zSafetyBaseId.abyData4[3]) AND(0u EQ pzInputSAPI->zSafetyBaseId.abyData4[4])
                        AND(0u EQ pzInputSAPI->zSafetyBaseId.abyData4[5])
                            AND(0u EQ pzInputSAPI->zSafetyBaseId.abyData4[6])
                                AND(0u EQ pzInputSAPI->zSafetyBaseId.abyData4[7]))
    {
        ...
        // SYSTEREL REVIEW : Same remark as uas_conv => this computation leads to intermediate U64 values. Use shifts!
        // Do not use 256uL !
        // Note that this is an example of implementation, but it should be made more readable using MACROS in
        // UAS_TYPES.h (Or revert to initial code...)
        pzResponseSpdu->zSpduId.dwPart2 =
            (((UAS_UInt32) pzSPI->zSafetyBaseId.wData2) + (((UAS_UInt32) pzSPI->zSafetyBaseId.wData3) << 16))
                XOR pzSPI->dwSafetyStructureSignature;
        /* SPDU_ID_3_i := BaseID (bytes 8…11) XOR BaseID (bytes 12…15) XOR ProviderID */
        pzResponseSpdu->zSpduId.dwPart3 =
            (((UAS_UInt32) pzSPI->zSafetyBaseId.abyData4[0]) + (((UAS_UInt32) pzSPI->zSafetyBaseId.abyData4[1]) << 8) +
             (((UAS_UInt32) pzSPI->zSafetyBaseId.abyData4[2]) << 16) +
             ((UAS_UInt32)(pzSPI->zSafetyBaseId.abyData4[3]) << 24))
                XOR(((UAS_UInt32) pzSPI->zSafetyBaseId.abyData4[4]) +
                    (((UAS_UInt32) pzSPI->zSafetyBaseId.abyData4[5]) << 8) +
                    (((UAS_UInt32) pzSPI->zSafetyBaseId.abyData4[6]) << 16) +
                    ((UAS_UInt32)(pzSPI->zSafetyBaseId.abyData4[7]) << 24));
    } /* if */
    else
    {
        pzResponseSpdu->zSpduId.dwPart1 = pzInputSAPI->zSafetyBaseId.dwData1 XOR UASDEF_SAFETY_PROVIDER_LEVEL_ID;
        pzResponseSpdu->zSpduId.dwPart2 =
            (((UAS_UInt32) pzInputSAPI->zSafetyBaseId.wData2) +
             (((UAS_UInt32) pzInputSAPI->zSafetyBaseId.wData3) << 16)) XOR pzSPI->dwSafetyStructureSignature;
        pzResponseSpdu->zSpduId.dwPart3 = (((UAS_UInt32) pzInputSAPI->zSafetyBaseId.abyData4[0]) +
                                           (((UAS_UInt32) pzInputSAPI->zSafetyBaseId.abyData4[1]) << 8) +
                                           (((UAS_UInt32) pzInputSAPI->zSafetyBaseId.abyData4[2]) << 16) +
                                           ((UAS_UInt32)(pzInputSAPI->zSafetyBaseId.abyData4[3]) << 24))
            XOR(((UAS_UInt32) pzInputSAPI->zSafetyBaseId.abyData4[4]) +
                (((UAS_UInt32) pzInputSAPI->zSafetyBaseId.abyData4[5]) << 8) +
                (((UAS_UInt32) pzInputSAPI->zSafetyBaseId.abyData4[6]) << 16) +
                ((UAS_UInt32)(pzInputSAPI->zSafetyBaseId.abyData4[7]) << 24));
    } /* else */
 ...
 */
static UAS_UInt32 byUASPROV_CalculateFcs(const UAS_SafetyProvider_type* const pzInstance)
{
    ...
    if (NULL NOT_EQ pzInstance)
    {
       ...
       instance parameter */
        ...
        // SYSTEREL REVIEW : No way to map a struct const * to a UAS_UInt8 *
        pbyParam = (const UAS_UInt8*) (&pzInstance->zInputSAPI.pbySerializedSafetyData);
        for (byIndex = 0u; byIndex < sizeof(UAS_UInt8*); byIndex++)
        {
            dwParamFcs += pbyParam[byIndex];
        } /* for */
        ...
        // SYSTEREL REVIEW : No way to map a struct const * to a UAS_UInt8 *
        pbyParam = (const UAS_UInt8*) (&pzInstance->zInputSAPI.pbySerializedNonSafetyData);
        for (byIndex = 0u; byIndex < sizeof(UAS_UInt8*); byIndex++)
        {
            dwParamFcs += pbyParam[byIndex];
        } /* for */
 ...
 */
static UAS_ParameterError_type nUASPROV_ValidInstanceData(const UAS_SafetyProvider_type* const pzInstance)
{
 ...
 * \return UAS_PARAMETER_OK if parameters are valid, or disgnosis information if parameters are invalid
 */
static UAS_ParameterError_type nUASPROV_ValidSPIParam(const UAS_SafetyProviderSPI_type* const pzSPI)
{
    UAS_ParameterError_type nResult = UAS_PARAMETER_OK;

    /*** check the BaseID ***/
    if ((0uL EQ pzSPI->zSafetyBaseId.dwData1) AND(0uL EQ pzSPI->zSafetyBaseId.wData2)
            AND(0uL EQ pzSPI->zSafetyBaseId.wData3) AND(0uL EQ pzSPI->zSafetyBaseId.abyData4[0])
                AND(0uL EQ pzSPI->zSafetyBaseId.abyData4[1]) AND(0uL EQ pzSPI->zSafetyBaseId.abyData4[2])
                    AND(0uL EQ pzSPI->zSafetyBaseId.abyData4[3]) AND(0uL EQ pzSPI->zSafetyBaseId.abyData4[4])
                        AND(0uL EQ pzSPI->zSafetyBaseId.abyData4[5]) AND(0uL EQ pzSPI->zSafetyBaseId.abyData4[6])
                            AND(0uL EQ pzSPI->zSafetyBaseId.abyData4[7]))
    {
