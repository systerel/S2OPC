 ...
 */
void vUASTIME_Init(UASTIME_Instance_type* const pzTimerInst,
 ...
 */
void vUASTIME_StartWatchdog(UASTIME_Instance_type* const pzTimerInst,
                            UASTIME_Instance_type* const r_pzTimerInst,
                            UASTIME_Timestamp_type const dwCurrentTime,
                            UAS_UInt32 const dwWatchdogTime)
{
    ...
    UASDEF_LOG_DEBUG("  vUASTIME_StartWatchdog: entry. dwCurrentTime = %ul, dwWatchdogTime =  %ul", ulCurrentTime,
                     dwWatchdogTime);
    ...
    if (bValid)
    {
        ...
        if ((UASTIME_Timestamp_type)(UASTIME_MAX - pzTimerInst->dwWatchdogTime) >= pzTimerInst->dwCurrentTime)
        {
            ...
            UASRVAR_SET_USIGN64(pzTimerInst->dwExpiringTime,
                                (pzTimerInst->dwCurrentTime + (UAS_UInt32) pzTimerInst->dwWatchdogTime));
        } /* if */
        else
        {
            ...
            UASRVAR_SET_USIGN64(
                pzTimerInst->dwExpiringTime,
                (((UAS_UInt32) pzTimerInst->dwWatchdogTime - (UASTIME_MAX - pzTimerInst->dwCurrentTime)) - 1u));
        } /* else */
    }     /* if */
    else
    {
 ...
 * \error handling: incrementation of wUASTIME_ErrorCount
 */
void vUASTIME_StartErrorIntervalLimit(UASTIME_Instance_type* const pzTimerInst,
                                      UASTIME_Instance_type* const r_pzTimerInst,
                                      UASTIME_Timestamp_type const dwCurrentTime,
                                      UAS_UInt16 const wErrorIntervalLimit)
{
    ...
    UASDEF_LOG_DEBUG("  vUASTIME_StartErrorIntervalLimit: entry. dwCurrentTime = %ul, wErrorIntervalLimit =  %u",
                     ulCurrentTime, dwWatchdogTime);
 ...
 * \error handling: incrementation of wUASTIME_ErrorCount
 */
void vUASTIME_Stop(UASTIME_Instance_type* const pzTimerInst,
                   UASTIME_Instance_type* const r_pzTimerInst,
                   UASTIME_Timestamp_type const dwCurrentTime)
{
#ifdef DBG_FDTIMER
 ...
 * \error handling: incrementation of wUASTIME_ErrorCount
 */
UAS_Bool bUASTIME_Expired(UASTIME_Instance_type* const pzTimerInst,   /**< Pointer to the primary instance data   */
...
)
{
    ...
    UASDEF_LOG_DEBUG("  vUASTIME_Stop: exit. bExpired = 0x%02X, wUASTIME_ErrorCount = 0x%04X", bExpired,
                     wUASTIME_ErrorCount);
 ...
 */
UAS_Bool bUASTIME_PreCheck(const UASTIME_Instance_type* const pzTimerInst,
