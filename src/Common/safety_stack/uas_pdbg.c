...
#define /*lint -e(961)*/ UASPDBG_GET_BIT(byte, bit) /*lint -e(835)*/ \
 ...
 */
void vUASPDBG_PrintBaseId(const UAS_Char* pszName,     /**< name of the BaseID parameter */
 ...
 */
void vUASPDBG_PrintSafetyProviderState(const UASPROV_StateMachine_type* const pzStateMachine)
{
    if (NULL NOT_EQ pzStateMachine)
    {
        ...
        UASDEF_LOG_DEBUG("        Handle                     = %u (0x%08X)", pzStateMachine->pzInstanceData->dwHandle,
                         pzStateMachine->pzInstanceData->dwHandle);
        UASDEF_LOG_DEBUG("        SafetyDataLength           = %u", pzStateMachine->pzInstanceData->wSafetyDataLength);
        UASDEF_LOG_DEBUG("        NonSafetyDataLength        = %u",
                         pzStateMachine->pzInstanceData->wNonSafetyDataLength);
        UASDEF_LOG_DEBUG("  ### SPI parameters");
        UASDEF_LOG_DEBUG("        SafetyProviderID           = %u (0x%08X)",
                         pzStateMachine->pzInstanceData->zSPI.dwSafetyProviderId,
                         pzStateMachine->pzInstanceData->zSPI.dwSafetyProviderId);
        ...
        UASDEF_LOG_DEBUG("        SafetyStructureSignature   = %u (0x%08X)",
                         pzStateMachine->pzInstanceData->zSPI.dwSafetyStructureSignature,
                         pzStateMachine->pzInstanceData->zSPI.dwSafetyStructureSignature);
        ...
        UASDEF_LOG_DATA("        SerializedSafetyData      ", pzStateMachine->pzInstanceData->wSafetyDataLength,
                        pzStateMachine->pzInstanceData->zInputSAPI.pbySerializedSafetyData);
        UASDEF_LOG_DATA("        SerializedNonSafetyData   ", pzStateMachine->pzInstanceData->wNonSafetyDataLength,
                        pzStateMachine->pzInstanceData->zInputSAPI.pbySerializedNonSafetyData);
        UASDEF_LOG_DEBUG("        EnableTestMode             = %u",
                         pzStateMachine->pzInstanceData->zInputSAPI.bEnableTestMode);
        UASDEF_LOG_DEBUG("        OperatorAckProvider        = %u",
                         pzStateMachine->pzInstanceData->zInputSAPI.bOperatorAckProvider);
        UASDEF_LOG_DEBUG("        ActivateFSV                = %u",
                         pzStateMachine->pzInstanceData->zInputSAPI.bActivateFsv);
        UASDEF_LOG_DEBUG("        SafetyProviderID           = %u (0x%08X)",
                         pzStateMachine->pzInstanceData->zInputSAPI.dwSafetyProviderId,
                         pzStateMachine->pzInstanceData->zInputSAPI.dwSafetyProviderId);
        vUASPDBG_PrintBaseId("        SafetyBaseID              ",
                             &pzStateMachine->pzInstanceData->zInputSAPI.zSafetyBaseId);
        UASDEF_LOG_DEBUG("  ### SAPI output parameters");
        UASDEF_LOG_DEBUG("        OperatorAckRequested       = %u",
                         pzStateMachine->pzInstanceData->zOutputSAPI.bOperatorAckRequested);
        UASDEF_LOG_DEBUG("        SafetyConsumerID           = %u (0x%08X)",
                         pzStateMachine->pzInstanceData->zOutputSAPI.dwSafetyConsumerId,
                         pzStateMachine->pzInstanceData->zOutputSAPI.dwSafetyConsumerId);
        UASDEF_LOG_DEBUG("        MonitoringNumber           = %u (0x%08X)",
                         pzStateMachine->pzInstanceData->zOutputSAPI.dwMonitoringNumber,
                         pzStateMachine->pzInstanceData->zOutputSAPI.dwMonitoringNumber);
        UASDEF_LOG_DEBUG("  ### RequestSPDU parameters");
        UASDEF_LOG_DEBUG("        SafetyConsumerID           = %u (0x%08X)",
                         pzStateMachine->pzInstanceData->zRequestSPDU.dwSafetyConsumerId,
                         pzStateMachine->pzInstanceData->zRequestSPDU.dwSafetyConsumerId);
        UASDEF_LOG_DEBUG("        MonitoringNumber           = %u (0x%08X)",
                         pzStateMachine->pzInstanceData->zRequestSPDU.dwMonitoringNumber,
                         pzStateMachine->pzInstanceData->zRequestSPDU.dwMonitoringNumber);
        UASDEF_LOG_DEBUG("        Flags                      = 0x%02X",
                         pzStateMachine->pzInstanceData->zRequestSPDU.byFlags);
        UASDEF_LOG_DEBUG(
            "        Flags.CommunicationError   = %u",
            UASPDBG_GET_BIT(pzStateMachine->pzInstanceData->zRequestSPDU.byFlags, UAS_BITPOS_COMMUNICATION_ERROR));
        UASDEF_LOG_DEBUG(
            "        Flags.OperatorAckRequested = %u",
            UASPDBG_GET_BIT(pzStateMachine->pzInstanceData->zRequestSPDU.byFlags, UAS_BITPOS_OPERATOR_ACK_REQUESTED));
        UASDEF_LOG_DEBUG(
            "        Flags.FSV_Activated        = %u",
            UASPDBG_GET_BIT(pzStateMachine->pzInstanceData->zRequestSPDU.byFlags, UAS_BITPOS_FSV_ACTIVATED));
        UASDEF_LOG_DEBUG("  ### ResponseSPDU parameters");
        UASDEF_LOG_DATA("        SerializedSafetyData      ", pzStateMachine->pzInstanceData->wSafetyDataLength,
                        pzStateMachine->pzInstanceData->zResponseSPDU.pbySerializedSafetyData);
        UASDEF_LOG_DEBUG("        Flags                      = 0x%02X",
                         pzStateMachine->pzInstanceData->zResponseSPDU.byFlags);
        UASDEF_LOG_DEBUG(
            "        Flags.OperatorAckProvider  = %u",
            UASPDBG_GET_BIT(pzStateMachine->pzInstanceData->zResponseSPDU.byFlags, UAS_BITPOS_OPERATOR_ACK_PROVIDER));
        UASDEF_LOG_DEBUG(
            "        Flags.ActivateFSV          = %u",
            UASPDBG_GET_BIT(pzStateMachine->pzInstanceData->zResponseSPDU.byFlags, UAS_BITPOS_ACTIVATE_FSV));
        UASDEF_LOG_DEBUG(
            "        Flags.TestModeActivated    = %u",
            UASPDBG_GET_BIT(pzStateMachine->pzInstanceData->zResponseSPDU.byFlags, UAS_BITPOS_TEST_MODE_ACTIVATED));
        UASDEF_LOG_DEBUG("        SPDU_ID                    = 0x%08X-%08X-%08X",
                         pzStateMachine->pzInstanceData->zResponseSPDU.zSpduId.dwPart1,
                         pzStateMachine->pzInstanceData->zResponseSPDU.zSpduId.dwPart2,
                         pzStateMachine->pzInstanceData->zResponseSPDU.zSpduId.dwPart3);
        UASDEF_LOG_DEBUG("        SafetyConsumerId           = %u (0x%08X)",
                         pzStateMachine->pzInstanceData->zResponseSPDU.dwSafetyConsumerId,
                         pzStateMachine->pzInstanceData->zResponseSPDU.dwSafetyConsumerId);
        UASDEF_LOG_DEBUG("        MonitoringNumber           = %u (0x%08X)",
                         pzStateMachine->pzInstanceData->zResponseSPDU.dwMonitoringNumber,
                         pzStateMachine->pzInstanceData->zResponseSPDU.dwMonitoringNumber);
        UASDEF_LOG_DEBUG("        CRC                        = 0x%08X",
                         pzStateMachine->pzInstanceData->zResponseSPDU.dwCrc);
        UASDEF_LOG_DATA("        SerializedNonSafetyData   ", pzStateMachine->pzInstanceData->wNonSafetyDataLength,
                        pzStateMachine->pzInstanceData->zResponseSPDU.pbySerializedNonSafetyData);
        ...
        else
        {
            UASDEF_LOG_DEBUG("        State                      = %u (%s)", pzStateMachine->nState,
                             pcSProvStateName[(UAS_Int16) pzStateMachine->nState - UASPDBG_FIRST_STATE]);
        } /* else */
 ...
 */
void vUASPDBG_PrintTransitionName(const UAS_Char* const pszTransitionName)
{
 ...
 */
void vUASPDBG_PrintBaseId(const UAS_Char* pszName, const UAS_GUID_type* pzBaseId)
{
