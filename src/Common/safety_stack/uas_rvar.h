...
/** Using an external variable is used to force the compiler not optimizing the COMPL (NULL)
 * because it may lead to warnings. (Non portable address)
 */
extern void* const UASRVAR_r_NULL;
...
/** Set redundant variable of type Pointer. */
#define UASRVAR_SET_NULL_POINTER(var) \
    (var) = NULL;                     \
    (r_##var) = UASRVAR_r_NULL
