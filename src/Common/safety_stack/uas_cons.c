...
#define /*lint -e(961)*/ UASCONS_GET_BIT(byte, bit) /*lint -e(835)*/ \
    (UAS_Bool)(((byte) BITAND(UAS_UInt8)(0x01u << (bit))) >> (bit))

/* This Makro is used to set the value of a single bit within a byte to 1. */
#define /*lint -e(960,961)*/ UASCONS_SET_BIT(byte, bit) /*lint -e(835)*/ \
    ((byte) = (UAS_UInt8)((byte) BITOR(UAS_UInt8)(0x01u << (bit))))

/* This Makro is used to set the value of a single bit within a byte to 0. */
#define /*lint -e(960,961)*/ UASCONS_DELETE_BIT(byte, bit) /*lint -e(835)*/ \
    ((byte) = (UAS_UInt8)((byte) BITAND(UAS_UInt8)(BITNOT(UAS_UInt8)(0x01u << (bit)))))
...
/* Use FSV */
static void vUASCONS_UseFsv(UAS_SafetyConsumer_type* const pzInstance, /**< Pointer to the instance data   */
                            UAS_Bool bConsumerTimerExpired /**< Indicates if the ConsumerTimer has been expired */
);

/* Use PV */
static void vUASCONS_UsePv(UAS_SafetyConsumer_type* const pzInstance /**< Pointer to the instance data   */
);
 ...
 */
UAS_UInt8 byUASCONS_Init(UASCONS_StateMachine_type* const pzStateMachine,
                         ...
                         UAS_ParameterError_type* const pnResult)
{
    ...
    if ((NULL EQ pzStateMachine) OR(NULL EQ r_pzStateMachine) OR(NULL EQ pzInstance) OR(NULL EQ pnResult))
    {
    ...
    else if ((0uL NOT_EQ pzStateMachine->dwParamFcs) OR(NULL NOT_EQ pzStateMachine->pzInstanceData))
    {
 ...
 * \return UASCONS_OK if ok, error code if error occured
 */
UAS_UInt8 byUASCONS_Reset(UASCONS_StateMachine_type* const pzStateMachine,
                          UASCONS_StateMachine_type* const r_pzStateMachine)
{
 ...
 * \return UASCONS_OK if ok, error code if error occured
 */
UAS_UInt8 byUASCONS_ChangeSPI(UASCONS_StateMachine_type* const pzStateMachine,
                              UASCONS_StateMachine_type* const r_pzStateMachine,
 ...
 */
UAS_UInt8 byUASCONS_ChangeTimeout(UASCONS_StateMachine_type* const pzStateMachine,
                                  UASCONS_StateMachine_type* const r_pzStateMachine,
 ...
 * \return UASCONS_OK if ok, error code if error occured
 */
UAS_UInt8 byUASCONS_Start(UASCONS_StateMachine_type* const pzStateMachine,
                          UASCONS_StateMachine_type* const r_pzStateMachine)
{
    /* check the redundant instance data */
    UAS_UInt8 byRetVal = byUASCONS_CheckRedundantData(pzStateMachine, r_pzStateMachine);
    ...
    if (UASCONS_OK EQ byRetVal)
    {
        if ((UASCONS_S10_INITIALIZE EQ pzStateMachine->nState) && (NULL NOT_EQ pzStateMachine->pzInstanceData) &&
 ...
 * \return UASCONS_OK if ok, error code if error occured
 */
UAS_UInt8 byUASCONS_Stop(UASCONS_StateMachine_type* const pzStateMachine,
                         UASCONS_StateMachine_type* const r_pzStateMachine)
{
    /* check the redundant instance data */
    UAS_UInt8 byRetVal = byUASCONS_CheckRedundantData(pzStateMachine, r_pzStateMachine);
 ...
 * \return UASCONS_OK if ok, error code if error occured
 */
UAS_UInt8 byUASCONS_Execute(UASCONS_StateMachine_type* const pzStateMachine,
                            UASCONS_StateMachine_type* const r_pzStateMachine)
{
    /* check the redundant instance data */
    UAS_UInt8 byRetVal = byUASCONS_CheckRedundantData(pzStateMachine, r_pzStateMachine);
 ...
 * \return UASCONS_OK if ok, error code if error occured
 */
static UAS_UInt8 byUASCONS_StateWaitForEvent(UASCONS_StateMachine_type* const pzStateMachine,
                                             UASCONS_StateMachine_type* const r_pzStateMachine)
{
    UAS_UInt8 byRetVal = UASCONS_DEFAULT_ERR;
...
#endif /* ifdef UASDEF_DBG */
                vUASTIME_StartErrorIntervalLimit(&pzStateMachine->zErrorIntervalTimer,
                                                 &r_pzStateMachine->zErrorIntervalTimer, dwCurrentTime,
                                                 pzInstance->zSPI.wSafetyErrorIntervalLimit);
...
#endif /* ifdef UASDEF_DBG */
                vUASTIME_StartWatchdog(&pzStateMachine->zConsumerTimer, &r_pzStateMachine->zConsumerTimer,
                                       dwCurrentTime, pzInstance->zSPI.dwSafetyConsumerTimeout);
    ...
    case UASCONS_S14_WAIT_FOR_CHANGED_SPDU:
    {
        ...
        pzStateMachine->bConsumerTimerExpired =
            bUASTIME_Expired(&pzStateMachine->zConsumerTimer, &r_pzStateMachine->zConsumerTimer, dwCurrentTime);
        pzStateMachine->bErrorIntervalTimerExpired = bUASTIME_Expired(
            &pzStateMachine->zErrorIntervalTimer, &r_pzStateMachine->zErrorIntervalTimer, dwCurrentTime);
        ...
        /*** T17 ***/
        else if ((bUASCONS_NoEmptySPDU(pzResponseSpdu)) &&
                 (pzResponseSpdu->dwMonitoringNumber NOT_EQ pzStateMachine->dwPrevMNR_i))
        {
#ifdef UASDEF_DBG
 ...
 * \return UASCONS_OK if ok, error code if error occured
 */
static UAS_UInt8 byUASCONS_StatePrepareRequest(UASCONS_StateMachine_type* const pzStateMachine,
                                               UASCONS_StateMachine_type* const r_pzStateMachine)
{
    UAS_UInt8 byRetVal = UASCONS_DEFAULT_ERR;
 ...
 * \return UASCONS_OK if ok, error code if error occured
 */
static UAS_UInt8 byUASCONS_StateCRCCheckSPDU(UASCONS_StateMachine_type* const pzStateMachine,
                                             UASCONS_StateMachine_type* const r_pzStateMachine)
{
    UAS_UInt8 byRetVal = UASCONS_DEFAULT_ERR;
    UASTIME_Timestamp_type dwCurrentTime = 0uLL; /*TODO*/
 ...
 * \return UASCONS_OK if ok, error code if error occured
 */
static UAS_UInt8 byUASCONS_StateCheckResponseSPDU(UASCONS_StateMachine_type* const pzStateMachine,
                                                  UASCONS_StateMachine_type* const r_pzStateMachine)
{
    UAS_UInt8 byRetVal = UASCONS_DEFAULT_ERR;
    UASTIME_Timestamp_type dwCurrentTime = 0uLL; /*TODO*/
    ...
    case UASCONS_S16_CHECK_RESPONSE_SPDU:
    {
        ...
        if ((pzResponseSpdu->zSpduId.dwPart1 EQ pzStateMachine->zSPDUID_i.dwPart1) AND(
                pzResponseSpdu->zSpduId.dwPart2 EQ pzStateMachine->zSPDUID_i.dwPart2)
                AND(pzResponseSpdu->zSpduId.dwPart3 EQ pzStateMachine->zSPDUID_i.dwPart3)
                    AND(pzResponseSpdu->dwSafetyConsumerId EQ pzStateMachine->dwConsumerID_i)
                        AND(pzResponseSpdu->dwMonitoringNumber EQ pzStateMachine->dwMNR_i))
        {
            ...
            pzInstance->zOutputSAPI.bOperatorAckProvider =
                UASCONS_DELETE_BIT(pzResponseSpdu->byFlags, UAS_BITPOS_OPERATOR_ACK_PROVIDER);
            ...
            if ((0u NOT_EQ pzInstance->zInputSAPI.bOperatorAckConsumer) AND(
                    0u NOT_EQ pzStateMachine->bOperatorAckConsumerAllowed_i))
            {
                pzInstance->zOutputSAPI.bOperatorAckRequested = 0u;
                UASCONS_DELETE_BIT(pzRequestSpdu->byFlags, UAS_BITPOS_OPERATOR_ACK_REQUESTED);
            } /* if */
            ...
            /* Set process values */
            if ((0u NOT_EQ pzInstance->zOutputSAPI.bOperatorAckRequested) OR(
                    0u NOT_EQ UASCONS_GET_BIT(pzResponseSpdu->byFlags, UAS_BITPOS_ACTIVATE_FSV)))
            {
                vUASCONS_UseFsv(pzInstance, pzStateMachine->bConsumerTimerExpired);
            } /* if */
            else
            ...
            pzInstance->zOutputSAPI.bTestModeActivated =
                UASCONS_GET_BIT(pzResponseSpdu->byFlags, UAS_BITPOS_TEST_MODE_ACTIVATED);
 ...
 * \return UASCONS_OK if ok, error code if error occured
 */
static UAS_UInt8 byUASCONS_StateError(UASCONS_StateMachine_type* const pzStateMachine,
                                      UASCONS_StateMachine_type* const r_pzStateMachine)
{
    UAS_UInt8 byRetVal = UASCONS_DEFAULT_ERR;
 ...
 * \return UASCONS_OK if ok, error code if error occured
 */
static UAS_UInt8 byUASCONS_StateProvideSafetyData(UASCONS_StateMachine_type* const pzStateMachine,
                                                  UASCONS_StateMachine_type* const r_pzStateMachine)
{
    UAS_UInt8 byRetVal = UASCONS_DEFAULT_ERR;
    UASTIME_Timestamp_type dwCurrentTime = 0uLL; /*TODO*/
...
#endif /* ifdef UASDEF_DBG */
            vUASTIME_StartWatchdog(&pzStateMachine->zConsumerTimer, &r_pzStateMachine->zConsumerTimer, dwCurrentTime,
                                   pzInstance->zSPI.dwSafetyConsumerTimeout);
            UASRVAR_SET_CONSSTATE(pzStateMachine->nState, UASCONS_S13_PREPARE_REQUEST);
            byRetVal = byUASCONS_StatePrepareRequest(pzStateMachine, r_pzStateMachine);
        } /* if */
...
)
{
    ...
    if ((0uL EQ pzInstance->zInputSAPI.zSafetyBaseId.dwData1) AND(0u EQ pzInstance->zInputSAPI.zSafetyBaseId.wData2)
            AND(0u EQ pzInstance->zInputSAPI.zSafetyBaseId.wData3)
                AND(0u EQ pzInstance->zInputSAPI.zSafetyBaseId.abyData4[0])
                    AND(0u EQ pzInstance->zInputSAPI.zSafetyBaseId.abyData4[1])
                        AND(0u EQ pzInstance->zInputSAPI.zSafetyBaseId.abyData4[2])
                            AND(0u EQ pzInstance->zInputSAPI.zSafetyBaseId.abyData4[3])
                                AND(0u EQ pzInstance->zInputSAPI.zSafetyBaseId.abyData4[4])
                                    AND(0u EQ pzInstance->zInputSAPI.zSafetyBaseId.abyData4[5])
                                        AND(0u EQ pzInstance->zInputSAPI.zSafetyBaseId.abyData4[6])
                                            AND(0u EQ pzInstance->zInputSAPI.zSafetyBaseId.abyData4[7]))
    {
    ...
    dwSpduId_x = (((UAS_UInt32) zBaseId.wData2) + (((UAS_UInt32) zBaseId.wData3) << 16))
                     XOR pzInstance->zSPI.dwSafetyStructureSignature;
    ...
    dwSpduId_x = (((UAS_UInt32) zBaseId.abyData4[0]) + (((UAS_UInt32) zBaseId.abyData4[1]) << 8) +
                  (((UAS_UInt32) zBaseId.abyData4[2]) << 16) + (((UAS_UInt32) zBaseId.abyData4[3]) << 24))
        XOR(((UAS_UInt32) zBaseId.abyData4[4]) + (((UAS_UInt32) zBaseId.abyData4[5]) << 8) +
            (((UAS_UInt32) zBaseId.abyData4[6]) << 16) + (((UAS_UInt32) zBaseId.abyData4[7]) << 24)) XOR dwProviderId;
 ...
 */
static void vUASCONS_UseFsv(UAS_SafetyConsumer_type* const pzInstance, UAS_Bool bConsumerTimerExpired)
{
    UAS_UInt16 wIndex;
 ...
 * \param[in/out]  pzInstance - Pointer to the instance data
 */
static void vUASCONS_UsePv(UAS_SafetyConsumer_type* const pzInstance)
{
    UAS_UInt16 wIndex;
    UAS_UInt16 wDataLength = pzInstance->wSafetyDataLength;
...
)
{
    ...
    if ((NULL EQ pzStateMachine) OR(NULL EQ r_pzStateMachine))
    {
        byRetVal = UASCONS_POINTER_ERR;
    } /* if */
 ...
 */
static UAS_UInt32 dwUASCONS_CalculateFcs(const UAS_SafetyConsumer_type* const pzInstance)
{
    UAS_UInt32 dwParamFcs = 0uL;

    if (NULL NOT_EQ pzInstance)
    {
        // SYSTEREL REVIEW : No way to map a struct const * to a UAS_UInt8 * (You MUST configure your compiler to show
        // that issue) pbyParam MUST be pointing to const
       ...
       block operations to calculate a frame check sequence across the static instance parameter */
        /*lint -e960 */ /* start of supress warning "Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other
       than array indexing used" because of block operations to calculate a frame check sequence across the static
       instance parameter */
 ...
 */
static UAS_ParameterError_type nUASCONS_ValidInstanceData(const UAS_SafetyConsumer_type* const pzInstance)
{
    UAS_ParameterError_type nResult = UAS_PARAMETER_OK;

    /***** Check SafetyData length *****/
    if ((0uL EQ pzInstance->wSafetyDataLength) OR(UASDEF_MAX_PV_LENGTH < pzInstance->wSafetyDataLength))
    {
        nResult = UAS_INVALID_SAFETY_DATA_LENGTH;
    } /* if */

    /***** Check NonSafetyData length *****/
    else if ((0uL EQ pzInstance->wNonSafetyDataLength) OR(UASDEF_MAX_PV_LENGTH < pzInstance->wNonSafetyDataLength))
    {
...
)
{
    ...
    if ((UAS_SAFETY_LEVEL_1 > pzSPI->bySafetyProviderLevel) OR(UAS_SAFETY_LEVEL_4 < pzSPI->bySafetyProviderLevel))
    {
    ...
    else if ((6u NOT_EQ pzSPI->wSafetyErrorIntervalLimit) AND(60u NOT_EQ pzSPI->wSafetyErrorIntervalLimit)
                 AND(600u NOT_EQ pzSPI->wSafetyErrorIntervalLimit))
    {
...
)
{
    ...
    if ((0 EQ pzSAPI->zSafetyBaseId.dwData1) AND(0 EQ pzSPI->zSafetyBaseId.dwData1)
            AND(0 EQ pzSAPI->zSafetyBaseId.wData2) AND(0 EQ pzSPI->zSafetyBaseId.wData2)
                AND(0 EQ pzSAPI->zSafetyBaseId.wData3) AND(0 EQ pzSPI->zSafetyBaseId.wData3)
                    AND(0 EQ pzSAPI->zSafetyBaseId.abyData4[0]) AND(0 EQ pzSPI->zSafetyBaseId.abyData4[0])
                        AND(0 EQ pzSAPI->zSafetyBaseId.abyData4[1]) AND(0 EQ pzSPI->zSafetyBaseId.abyData4[1])
                            AND(0 EQ pzSAPI->zSafetyBaseId.abyData4[2]) AND(0 EQ pzSPI->zSafetyBaseId.abyData4[2])
                                AND(0 EQ pzSAPI->zSafetyBaseId.abyData4[3]) AND(0 EQ pzSPI->zSafetyBaseId.abyData4[3])
                                    AND(0 EQ pzSAPI->zSafetyBaseId.abyData4[4])
                                        AND(0 EQ pzSPI->zSafetyBaseId.abyData4[4])
                                            AND(0 EQ pzSAPI->zSafetyBaseId.abyData4[5])
                                                AND(0 EQ pzSPI->zSafetyBaseId.abyData4[5])
                                                    AND(0 EQ pzSAPI->zSafetyBaseId.abyData4[6])
                                                        AND(0 EQ pzSPI->zSafetyBaseId.abyData4[6])
                                                            AND(0 EQ pzSAPI->zSafetyBaseId.abyData4[7])
                                                                AND(0 EQ pzSPI->zSafetyBaseId.abyData4[7]))
    {
        bResult = 0;
    } /* if */
 ...
 */
static UAS_Bool bUASCONS_NoEmptySPDU(const UAS_ResponseSpdu_type* const pzResponseSpdu)
{
    UAS_Bool bRetVal = 0;
