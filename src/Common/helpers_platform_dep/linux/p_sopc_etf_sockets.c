...
/*
 Earliest TxTime First (ETF) - Socket abstraction allows applications to control
 the instant when a packet should be dequeued from the traffic control layer into the netdevice.

 There is two mode of transmission strict or deadline. In Strict mode packet will be dequeued
 at exact launch time, deadline mode packet will be dequeued anytime within the timeslice.

 The socket abstraction is intented to work over a traffic control layer configured to handle ETF.
*/
...
SOPC_ReturnStatus SOPC_ETF_Socket_Option(Socket* sock, bool deadlineMode, uint32_t soPriority, bool errorQueueEnable)
{
    ...
    */
    ...
    SOPC_ETF_Socket_TxTime txtimeSock;
    const int trueInt = true;
    int setOptStatus = -1;
    uint16_t deadlineMask = 0;
    uint16_t receiveErrors = 0;
...
SOPC_ReturnStatus SOPC_ETF_Socket_send(Socket sock, SOPC_Buffer* buffer, uint64_t txtime, SOPC_Socket_Address* sockAddr)
{
    ...
    S2OPC_TEMP_FAILURE_RETRY(res, sendmsg(sock, &message, 0));
...
SOPC_ReturnStatus SOPC_ETF_Socket_Error_Queue(Socket sock)
{
    ...
    {
        S2OPC_TEMP_FAILURE_RETRY(res, recvmsg(sock, &message, MSG_ERRQUEUE));
        ...
        {
            controlMessage = CMSG_FIRSTHDR(&message);
            while (controlMessage != NULL && status == SOPC_STATUS_OK)
            {
                memcpy(CMSG_DATA(controlMessage), &sockErr, sizeof(struct sock_extended_err));
                ...
                */
                    ...
                    */
                    ...
                    case SO_EE_CODE_TXTIME_MISSED:
                        SOPC_Logger_TraceWarning(SOPC_LOG_MODULE_COMMON,
                                                 "Packet with timestamp %" PRIu64 " dropped ee_code = %d", timestamp,
                                                 sockErr.ee_code);
                        status = SOPC_STATUS_NOK;
                        break;
                    default:
                ...
                else
                {
                    ...
                    status = SOPC_STATUS_NOK;
                    SOPC_Logger_TraceWarning(SOPC_LOG_MODULE_COMMON,
                                             "Unknown error on etf sending. sockErr.ee_origin  = %d",
                                             sockErr.ee_origin);
                }
            }
        }
