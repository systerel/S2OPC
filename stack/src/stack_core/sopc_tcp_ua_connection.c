...
void TCP_UA_Connection_EndSocketCreation_CB(void*           cbData,
                                            SOPC_StatusCode creationStatus,
                                            SOPC_Socket*    newSocket)
{
    TCP_UA_Connection* connection = (TCP_UA_Connection*) cbData;
    if(NULL != connection){
        if(STATUS_OK == creationStatus){
            connection->socket = newSocket;
        }
        if(STATUS_OK == creationStatus){
            creationStatus = InitSendBuffer(connection);
        }
        if(STATUS_OK == creationStatus){
            creationStatus = InitReceiveBuffer(connection);
        }

        if(STATUS_OK != creationStatus && connection->callback != NULL){
            // Notify only in case of connection creation failure,
            // in case of success we are waiting for the TCP UA handshake (HEL, ACK)
...
SOPC_StatusCode SendHelloMsg(TCP_UA_Connection*           connection,
                             SOPC_Socket_EndOperation_CB* cb,
                             void*                        cbData){
    ...
    if(status == STATUS_OK){
        status = SOPC_UInt32_Write(&connection->protocolVersion,
                              connection->outputMsgBuffer->buffers);
    }
    if(status == STATUS_OK){
    ...
    if(connection != NULL
       && connection->inputMsgBuffer != NULL){
        if(connection->inputMsgBuffer->currentChunkSize == TCP_UA_ACK_MSG_LENGTH){
            ...
            status = SOPC_UInt32_Read(&tempValue, connection->inputMsgBuffer->buffers);
            if(status == STATUS_OK){
       ...
       && connection->inputMsgBuffer != NULL){
        if(connection->inputMsgBuffer->currentChunkSize > TCP_UA_HEL_MSG_LENGTH &&
           connection->inputMsgBuffer->currentChunkSize <= TCP_UA_HEL_MSG_MAX_LENGTH){
            ...
            // EndpointURL
            if(STATUS_OK == status){
                status = SOPC_String_Read(&url, connection->inputMsgBuffer->buffers);
                if(STATUS_OK == status){
...
void SOPC_OperationEnd_TCP_UA_ConnectionError(void*           arg,
...
void SOPC_OperationEnd_TCP_UA_Connection(void*           arg,
                                         SOPC_StatusCode status)
{
    TCP_UA_Connection* connection = (TCP_UA_Connection*) arg;
    if(STATUS_OK == status){
        ...
        SOPC_OperationEnd_TCP_UA_ConnectionError(arg, status);
    }
}
...
SOPC_StatusCode ReceiveErrorMsg(TCP_UA_Connection* connection){
    ...
    if(connection != NULL && connection->inputMsgBuffer != NULL)
    {
        ...
        {
            // Read error cpde
            status = SOPC_UInt32_Read(&error, connection->inputMsgBuffer->buffers);
            if(status == STATUS_OK){
                status = error;
                tmpStatus = SOPC_String_Read(reason, connection->inputMsgBuffer->buffers);
       ...
       callback != NULL)
    {
           ...
           connection->state == TCP_UA_Connection_Disconnected)
        {
            ...
            if(status == STATUS_OK){
                ...
                status = SOPC_CreateAction_SocketCreateClient(uri,
                                                              OnSocketEvent_CB,
                                                              (void*) connection,
                                                              TCP_UA_Connection_EndSocketCreation_CB,
                                                              (void*) connection);
            }
