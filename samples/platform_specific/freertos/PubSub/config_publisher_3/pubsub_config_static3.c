...
SOPC_PubSubConfiguration* SOPC_PubSubConfig_GetStatic(void)
{
    ...
    if (alloc)
    {
        // GroupId = 15
        // GroupVersion = 1
        // Interval = 300.000000 ms
        // Offest = -1 us
 
       writerGroup = SOPC_PubSubConfig_SetPubMessageAt(connection, 1, 15, 1, 300.000000,-1, SOPC_SecurityMode_None);
       alloc = NULL != writerGroup;
    }
    ...
    if (alloc)
    {
        ...
        SOPC_ASSERT(NULL != writer);
        // WriterId = 52
        dataset = SOPC_PubSubConfig_InitDataSet(config, 2, writer, 0, 52, 2);
        alloc = NULL != dataset;
    }
    if (alloc)
    {
