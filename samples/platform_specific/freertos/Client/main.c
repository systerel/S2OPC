...
int FREE_RTOS_S2OPC_CLIENT(void* ptr);
...
int main(void)
{
    ...
    // Create S2OPC client Thread
    FREE_RTOS_S2OPC_CLIENT(NULL);
...
int FREE_RTOS_S2OPC_CLIENT(void* ptr)
{
    ...
    Thread pX = NULL;
    status = SOPC_Thread_Create(&pX, (void*) cbToolkit_test_client, pv, NULL);
    return status;
}
