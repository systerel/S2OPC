...
/** \file
 *
 * \brief A client executable using the s2opc_clientserver subscription lib
 *
 */
...
int main(int argc, char* const argv[])
{
    ...
    if (cfg_con.security_mode != OpcUa_MessageSecurityMode_None)
    {
        status = SOPC_ClientConfigHelper_SetClientKeyPasswordCallback(&client_get_private_key_password);
