How to generate `s2opc.xml`, `s2opc_nano.xml` and `s2opc_sks.xml`: the 2 first files are generated from `s2opc_base_nodeset_origin.xml` and `s2opc_demo_data_origin.xml` using `nodeset-address-space-utils.py` tool
